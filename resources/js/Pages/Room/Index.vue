...
const store = () => {
  return form.post(route('room.store'), {
    onSuccess: () => close(),
    onError: () => show(),
  })
}
...
const edit = room => {
  ...
  form.name = room.name
  form.code = room.code
  form.majors = room.permissions.map(major => major.id)

  show()
}

const update = () => {
  return form.patch(route('room.update', form.id), {
    onSuccess: () => close(),
    onError: () => show(),
  })
}
...
const destroy = async room => {
  ...
  if (response.isConfirmed) {
    ...
    return Inertia.delete(route('room.destroy', room.id))
  }
}

const submit = () => form.id ? update() : store()

// const detachPermission = async (room, permission) => {
//   const response = await Swal.fire({
//     title: __('are you sure') + '?',
...
//   Inertia.on('finish', () => close())
//   Inertia.patch(route('room.permission.detach', { room: room.id, permission: permission.id }))
// }

// const detachRole = async (room, role) => {
//   const response = await Swal.fire({
//     title: __('are you sure') + '?',
//     text: __('you can re adding it on edit page'),
//     icon: 'question',
...
//   Inertia.on('finish', () => close())
//   Inertia.patch(route('room.role.detach', { room: room.id, role: role.id }))
// }
              ...
              >
                ...
                <template v-else>
                  ...
                  >
                      ...
                      <div class="flex-wrap">
                        ...
                        >
                          <div class="flex items-center justify-between space-x-1">
                            ...
                            <Icon
                              v-if="can('update room')"
                              @click.prevent="detachMajor(room major)"
                              name="times"
                    ...
                    <td class="px-2 py-1 border dark:border-gray-800 uppercase">
                      {{ new Date(room.created_at).toLocaleString('id') }}
                    </td>

                    <td class="px-2 py-1 border dark:border-gray-800 uppercase">
                      {{ new Date(room.updated_at).toLocaleString('id') }}
                    </td>
        ...
        <Card class="bg-gray-50 dark:bg-gray-700 dark:text-gray-100">
          ...
          <template #body>
            <div class="flex flex-col space-y-4 p-4">
              ...
              <div class="flex flex-col space-y-2">
                <div class="flex items-center space-x-2">
                  <label for="majors" class="w-1/3 lowercase first-letter:capitalize">
                    {{ __('jurusan') }}
                  </label>

                  <Select
                    v-model="form.majors"
                    :options="majors.map(p => ({
                      label: __(p.name),
                      value: p.id,
                    }))"
                    :clearOnSelect="false"
                ...
                <InputError :error="form.errors.majors" />
              </div>
            </div>
          </template>
