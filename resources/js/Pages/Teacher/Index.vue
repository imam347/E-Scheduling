...
const store = () => {
  return form.post(route('teacher.store'), {
    onSuccess: () => close(),
    onError: () => show(),
  })
}
...
const update = () => {
  return form.patch(route('teacher.update', form.id), {
    onSuccess: () => close(),
    onError: () => show(),
  })
}

const destroy = async teacher => {
  ...
  if (response.isConfirmed) {
    ...
    return Inertia.delete(route('teacher.destroy', teacher.id))
  }
}

const submit = () => form.id ? update() : store()

// const detachPermission = async (teacher, permission) => {
//   const response = await Swal.fire({
//     title: __('are you sure') + '?',
...
//   Inertia.on('finish', () => close())
//   Inertia.patch(route('supermajor.teacher.permission.detach', { teacher: teacher.id, permission: permission.id }))
// }

// const detachRole = async (teacher, role) => {
//   const response = await Swal.fire({
//     title: __('are you sure') + '?',
//     text: __('you can re adding it on edit page'),
//     icon: 'question',
...
//   Inertia.on('finish', () => close())
//   Inertia.patch(route('supermajor.teacher.role.detach', { teacher: teacher.id, role: role.id }))
// }
              ...
              >
                ...
                <template v-else>
                  ...
                  >
                    ...
                    <td class="px-2 py-1 border dark:border-gray-800 uppercase">
                      {{ new Date(teacher.created_at).toLocaleString('id') }}
                    </td>

                    <td class="px-2 py-1 border dark:border-gray-800 uppercase">
                      {{ new Date(teacher.updated_at).toLocaleString('id') }}
                    </td>
