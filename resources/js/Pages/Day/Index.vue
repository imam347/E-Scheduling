...
const store = () => {
  return form.post(route('day.store'), {
    onSuccess: () => close(),
    onError: () => show(),
  })
}
...
const update = () => {
  return form.patch(route('day.update', form.id), {
    onSuccess: () => close(),
    onError: () => show(),
  })
}

const destroy = async day => {
  ...
  if (response.isConfirmed) {
    ...
    return Inertia.delete(route('day.destroy', day.id))
  }
}

const submit = () => form.id ? update() : store()

// const detachPermission = async (day, permission) => {
//   const response = await Swal.fire({
//     title: __('are you sure') + '?',
...
//   Inertia.on('finish', () => close())
//   Inertia.patch(route('supermajor.day.permission.detach', { day: day.id, permission: permission.id }))
// }

// const detachRole = async (day, role) => {
//   const response = await Swal.fire({
//     title: __('are you sure') + '?',
//     text: __('you can re adding it on edit page'),
//     icon: 'question',
...
//   Inertia.on('finish', () => close())
//   Inertia.patch(route('supermajor.day.role.detach', { day: day.id, role: role.id }))
// }
              ...
              >
                ...
                <template v-else>
                  ...
                  >
                    ...
                    <td class="px-2 py-1 border dark:border-gray-800 uppercase">
                      {{ new Date(day.created_at).toLocaleString('id') }}
                    </td>

                    <td class="px-2 py-1 border dark:border-gray-800 uppercase">
                      {{ new Date(day.updated_at).toLocaleString('id') }}
                    </td>
